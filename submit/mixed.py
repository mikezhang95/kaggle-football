...
def get_closet_to_ball(obs):
    """ get average distance to closest opponents """
    ball_x = obs["ball"][0]
    ball_y = obs["ball"][1]
    player, dist = 0, 100
    for i in range(0, len(obs["left_team"])):
        x, y = obs["left_team"][i][0], obs["left_team"][i][1]
        distance_to_ball = get_distance(x, y, ball_x, ball_y)
        if distance_to_ball < dist:
            dist = distance_to_ball
            player = i
    obs["to_ball_distance"] = dist
    obs["to_ball_player"] = player
...
def close_to_goalkeeper_shot(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        obs["to_keeper"] = get_distance(player_x, player_y, goalkeeper_x, goalkeeper_y) 
        if obs["to_keeper"] < 0.3 and obs["to_keeper"] > 0.05: # too close
            return True
        return False
        
    def get_action(obs, player_x, player_y):
        """ get action of this memory pattern """
        # rl shoot: mask other irrelevant agents 
        if player_x > 0.7 :
            raw_obs = obs["raw_obs"]     
            real_active = raw_obs["players_raw"][0]["active"]
            left_team = raw_obs["players_raw"][0]["left_team"]
            right_team = raw_obs["players_raw"][0]["right_team"]
            raw_obs["players_raw"][0]["left_team"] = np.array([left_team[0], left_team[real_active]])
            raw_obs["players_raw"][0]["right_team"] = np.array([right_team[0]])
            raw_obs["players_raw"][0]["active"] = 1
...
def offence_memory_patterns(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        to_ball_distance = obs["to_ball_distance"]
        # 有球，或者长传，或者射门
        if (obs["ball_owned_team"] == 0) or (obs["ball_owned_team"] != 1 and ( high_pass_state or to_ball_distance < 0.03) ): 
            return True
        return False
...
def defence_memory_patterns(obs, player_x, player_y):
    ...
    def get_memory_patterns(obs, player_x, player_y):
        ...
        if obs["ball_owned_team"] == 1:
            obs["memory_patterns"]["ball_next_coords"]["x"] -= 0.03   
...
def agent(raw_obs):
    ...
    # 1. 计算到球最近距离的球员，和距离
    get_closet_to_ball(obs)
    
    # 2. 这个球员可能是active的
    if obs["raw_obs"]["players_raw"][0]["active"] == -1:
        obs["raw_obs"]["players_raw"][0]["active"] = obs["to_ball_player"] # closet player is activate
        obs["active"] = obs["to_ball_player"]
